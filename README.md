...
https://coconala.com/services/3724976

なお、Wordpressプラグインとしては現状無償です

## アンインストール
- 無効化して削除

## バージョン履歴
v0.4.5 - 他ドメインへのコピーやローカルへのコピー対処をした

v0.4.4 - 軽易な変更

v0.4.2 - jsのミス修正

v0.4.1 - 利用規約ページを設定できるようにした

v0.4.0 - 画像保護時の動的ウォーターマークに対応

v0.3.16 - 0.3.15で追加した機能を警告できるようにした

v0.3.15 - ほとんどのブラウザーでデバッガーをマウスで開いてもログに残るようにした（警告なし）

v0.3.14 - UA不正の場合の条件が逆だったのを修正

v0.3.13 - ウェブアーカイブ、魚拓をアクセス禁止にできるようにした

v0.3.12 - 特許に関することを管理画面にも表示するようにした

v0.3.11 - 隠しコマンド ショートコード [neohp_iplog_reader]を追加、デモサイト用に追加したものなので、通常利用には適しない、その他ライセンスを明確に変更

v0.3.9 - EU GDPRに準拠しない可能性があるため、プライバシーポリシーのページだけを普通にblurしないようにした

v0.3.8 - 同意画面の言語を設定できるようにした

v0.3.7 - アンインストールするとWordpressで設定されているプライバシーポリシーが消えるのを修正

v0.3.6 - 同意画面等のぼかしと透過、その他ダークモードの設定をできるようにした

v0.3.5 - 同意画面等にぼかしと透過を入れた

v0.3.4 - Fireタブレットも検出、同意画面等がデバイスに表示しきれないのを修正

v0.3.3 - iPad/AndroidタブレットをJSで検出可能にした

v0.3.2 - 同意画面等がiOS Safariで動作しなかったのを修正、その他画面調整

v0.3.1 - 同意画面をカスタマイズ可能にした

v0.3.0 - EU GDPRに対応するように、同意画面を追加

v0.2.25 - 過去に3.5-turboで作ったMRI音を4-turboで作り直したものを警告BGMに追加
