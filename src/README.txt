...
== Special Note on Patents ==
このプラグインは特許出願中の技術を実装しています。

特許取得済みの機能には以下が含まれる。
- view-sourceロギング
- CookieとHTMLリダイレクトによるHTML保護システム
- スクリーンショットの予兆キーの検出
- 画像URLをワンタイムURLにして再利用できないシステム

WordPress.orgのプラグインディレクトリからこのプラグインを使用または配布することにより、作者はGNU General Public License v2の条項の下、これらの特許技術を「このプラグイン内でのみ」使用する非独占的、使用料無料（プレミアム機能はプラグイン内で提示されている価格で）、取り消し不能のライセンスを付与します。

これらの技術を「このプラグイン以外」で使用するには、別途商用ライセンスが必要な場合があります。
